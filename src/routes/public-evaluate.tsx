 ...
 *
 * OWNER: B3. Placeholder — replace the contents, keep the exported app
...
app.get('/:event/evaluate', async (c) => {
  ...
  return c.html(
    <PublicLayout title="Evaluator workspace" event={found.event} theme={found.theme} maxWidth={680} toast={c.req.query('ok') ?? null}>
      <div style={`max-width:680px;margin:0 auto;padding:48px 20px 80px;`}>
        <div style="background:var(--card);border:1px solid var(--border);padding:40px 24px;text-align:center;">
          ...
          <div style="font-size:19px;font-weight:700;letter-spacing:-0.01em;margin-bottom:4px;">Evaluator workspace</div>
          <div style="font-size:13.5px;color:var(--text-secondary);">The review queue, star rubric and blind review land in track B3.</div>
        </div>
      </div>
    </PublicLayout>
  );
