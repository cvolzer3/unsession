...
- **Keyboard:** in the admin, `⌘K` (`Ctrl+K`) opens the command palette — the last three submissions on top, then every page; `⌘L` opens it on the submissions alone. Each page also has a direct shortcut (for example `⌘S` for Submissions) — the palette shows them.
