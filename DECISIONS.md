...
- **R14 · Deciding and informing speakers are separate steps** (the Sessionboard/Sessionize pattern, Chris-requested). The decision modal — including the evaluation view's Approve/Waitlist/Deny buttons, which route into it — now only QUEUES a decision (`decision_queue` table, `lib/decision-queue.ts`): no status flip, no session copy, no tasks, no email, nothing visible on the speaker portal, freely revertible. Organizers review and send from **Emails → Outbox**, which runs `applyDecision` with all side effects in the usual order — so confirmation-link TTL and the task digest both start at send time. One queued decision per submission; re-queueing replaces it ("latest decision wins"). Sends go out ≤40 per click to fit a request's subrequest budget; the redirect reports the remainder. Chris's calls: queue-only (no "send now" escape hatch), outbox lives on the Emails page, evaluator reminders keep sending immediately. Sessions are created at send time, not queue time (Sessionboard makes queued accepts visible to the agenda team — revisit if agenda planning wants tentative sessions earlier). The public API v1 / MCP `decide_submission` still applies decisions immediately — machine callers are explicit; flag if the queue should cover them too.
