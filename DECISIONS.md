...
## Decision outbox (2026-08-12, with Chris)

- **R14 · Deciding and informing speakers are separate steps** (the Sessionboard/Sessionize pattern, Chris-requested). The decision modal — including the evaluation view's Approve/Waitlist/Deny buttons, which route into it — now only QUEUES a decision (`decision_queue` table, `lib/decision-queue.ts`): no status flip, no session copy, no tasks, no email, nothing visible on the speaker portal, freely revertible. Organizers review and send from **Emails → Outbox**, which runs `applyDecision` with all side effects in the usual order; confirmation-link TTL and the task digest both start at send time. One queued decision per submission; re-queueing replaces it ("latest decision wins"). Sends go out ≤40 per click to fit a request's subrequest budget; the redirect reports the remainder. Chris's calls: queue-only (no "send now" escape hatch), outbox lives on the Emails page, evaluator reminders keep sending immediately. Sessions are created at send time, not queue time (Sessionboard makes queued accepts visible to the agenda team; revisit if agenda planning wants tentative sessions earlier). The public API v1 / MCP `decide_submission` still applies decisions immediately; flag if the queue should cover machine callers too.
...
- **D12 · Google OAuth ships dark** until you create a GCP OAuth client (needs your Google account). The button appears automatically once `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` secrets are set. **[Superseded 2026-08-12 — see D14; Google OAuth is removed, secrets and all.]**
...
- **D14 · Auth is email + password for everyone (2026-08-12, your call; supersedes D2 and D12).** Passwordless was my simplification, not yours; you asked for conventional auth, so the product now signs everyone in with an email and a password. New pages: `/signup`, `/auth/forgot`, `/auth/reset`, `/auth/set-password`. Passwords are PBKDF2-SHA256 via WebCrypto; `users.google_id` is dropped and `users.password_hash` added (`migrations/0017_password_auth.sql`). Google OAuth is gone entirely: routes, button, and the `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` secrets. This closes Q1. Magic tokens survive, but only for things that were never sign-in: team invites (accepting routes a new user through password setup), participation confirmations (`/confirm`), draft-resume links, and password reset / first-time setup. The `'signin'` purpose is deleted. Accounts auto-created by the draft/submission flow (D3) still auto-create — they just set a password through the forgot/reset flow, which doubles as first-time setup.

- **D15 · The MCP endpoint speaks OAuth 2.1 with Dynamic Client Registration (2026-08-12).** Static `uns_…` tokens stay the primary API story, but MCP clients that expect an OAuth "Connect" flow (claude.ai connectors, VS Code, Cursor) now get one: RFC 9728 resource metadata (pointed to by the `WWW-Authenticate` header on API 401s), RFC 8414 server metadata, RFC 7591 registration (public clients, PKCE S256 only, no client secrets), a `/oauth/authorize` consent page on top of the password session (pick a workspace you own/admin plus read or read-write scope; sandbox orgs excluded, same rule as `/app/api`), and `/oauth/token` minting ordinary `api_tokens` rows — `oauth_client_id` set, 1-hour access secrets, refresh tokens (`unsr_…`) rotated on every refresh. Because OAuth mints into `api_tokens`, Bearer auth, the `/app/api` list and Revoke needed no structural changes, and revoking the row kills the whole connection. Migration `0026_oauth_dcr.sql`; routes in `src/routes/oauth.tsx`.

## Questions for you (when you're back)

- **Q1 — Google OAuth: MOOT, nothing to do.** (Was: create a GCP OAuth client, or should I walk you through it live?) You chose email+password sign-in instead; see D14.
- **Q2 — Real email sending (the one decision that costs money):** Cloudflare **Email Sending requires the Workers Paid plan ($5/month)**. I stopped at that screen since you authorized free tier only. Until then the app runs in "simulated email" mode: every email is recorded in the email log, and magic links are surfaced on-screen, so all flows work end to end today. Options: **(a) Workers Paid $5/mo — my recommendation** (native sending from unsession.dev, plus 30s CPU limit instead of 10ms, Queues, higher limits); (b) free Resend account (100 emails/day): you'd create the account + API key, and I've built the email layer so it plugs in with two secrets; (c) stay simulated for now.
